...
# IMPORTANT!!!
# need to run the following if using flatpak anki:
#sudo flatpak override net.ankiweb.Anki --talk-name=org.freedesktop.Flatpak
#sudo flatpak override net.ankiweb.Anki --filesystem=~/.var/app/net.ankiweb.Anki/data/Anki2/addons21/976516370/user_files/
...
def show_popUp(cnt, ease):
    ...
    def launchpqiv(image_path): #pqiv
        os.system(f'flatpak-spawn --host pqiv --bind-key="<space> {{ quit(); }}" -fci "{image_path}" &') #pwiv
    if not show_image and not show_header:
        return
    elif ease == 1 and show_onAgain:
        launchpqiv(image_path) #pwiv
        # window.exec()
